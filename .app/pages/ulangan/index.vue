...
import type { dailyExam } from '../../types'
...
const { data, pending, error, refresh } = await useFetch<DailyExamResponse>(`${config.public.apiUrl}/student-daily-exams`, {
  headers: {
    ...
    <div>
      ...
      <template v-else>
        <div v-if="data?.data.length === 0">
          <BasePlaceholderPage title="Ulangan tidak ditemukan" subtitle="Sepertinya sedang tidak ada ulangan saat ini.">
            <template #image>
        ...
        <div v-else>
          <div class="ltablet:grid-cols-3 grid w-full gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <BaseCard v-for="item in data?.data" :key="item.id" shape="curved" class="p-4">
              <div>
                <img :src="getRandomImage()" :alt="item.title" class="rounded-lg" />
              </div>
              <div class="my-4 flex items-center justify-between">
                <div>
                  <h4 class="mb-3 text-muted-800 dark:text-muted-100 font-sans text-base font-medium">
              ...
              <div class="flex items-center gap-2">
                <BaseButton :to="`/ulangan/${item.id}`" shape="curved" v-bind="buttonClass(item.start_in,item.finished)" class="w-full">
